...
pub fn encode(line: *Tokenizer, diag: *Diagnostic) !?Self {
...
fn encodeDataTransfer(line: *Tokenizer, diag: *Diagnostic, comptime flags: DataTransfer) !Self {
    var inst = flags;

    inst.rd = try parsing.expectRegister(line, diag);

    _ = try parsing.expectOperator(",", line, diag);
    _ = try parsing.expectOperator("[", line, diag);

    inst.rm = try parsing.expectRegister(line, diag);
    ...
    if (parsing.expectRegisterOrValue(line, diag)) |op| switch (op) {
        ...
        .imm => |imm| {
            ...
            if (inst.p == 1) return .{ .encoding = @bitCast(inst) };
        },
        else => unreachable,
    } else |err| switch (err) {
    ...
    if (inst.p == 0) {
        if (parsing.optionalOperator("]", line) == ']') {
            if (parsing.optionalOperator("!", line) == '!') inst.w = 1;
            return .{ .encoding = @bitCast(inst) };
        }
    }
    ...
    _ = try parsing.expectOperator("]", line, diag);
    if (parsing.optionalOperator("!", line) != '!') return .{ .encoding = @bitCast(inst) };

    inst.w = 1;
    return .{ .encoding = @bitCast(inst) };
}

fn encodeMoveFromPsr(line: *Tokenizer, diag: *Diagnostic) !Self {
    return .{ .encoding = @bitCast(MoveFromPsr{ .rd = try parsing.expectRegister(line, diag) }) };
}

fn encodeSetClearPsrBits(line: *Tokenizer, diag: *Diagnostic, flags: SetClearPsrBits) !Self {
    var inst = flags;
...
fn encodeDataProcessing(mnem: u32, line: *Tokenizer, diag: *Diagnostic, flags: DataProcessing) !Self {
    var inst = flags;

    // NOP
    if (mnem == perf.NOP) return .{ .encoding = @bitCast(inst) };
    ...
    if (mnem >= perf.LSL and mnem <= perf.LSLX) {
        inst.operand2 = try parsing.expectRegister(line, diag);
        _ = try parsing.expectOperator(",", line, diag);
    } else {
        inst.rm = try parsing.expectRegister(line, diag);
...
fn encodeBranch(line: *Tokenizer, diag: *Diagnostic, flags: Branch) !Self {
    var inst = flags;
    ...
    if (parsing.parseOperand(token)) |op| switch (op) {
        ...
        .label => |label| return .{ .encoding = @bitCast(inst), .reloc = label },
        else => {
            try diag.msg("expected register or label\n", .{});
            return error.ParseError;
        },
...
fn encodeMoveImmediate(line: *Tokenizer, diag: *Diagnostic) !Self {
    ...
    // Test for optional '-'
    if (parsing.optionalOperator("-", line) == '-') inst.m = 1;
...
fn encodeSoftwareInterrupt(line: *Tokenizer, diag: *Diagnostic) !Self {
    var inst = SoftwareInterrupt{};

    const token = line.next() orelse return .{ .encoding = @bitCast(inst) };
...
fn encodeMove32(line: *Tokenizer, diag: *Diagnostic) !Self {
...
test "Empty lines" {
    ...
    try std.testing.expectEqual(null, try Self.encode(&line, &err));

...
pub fn encodeInstruction(line: *Tokenizer, diag: *Diagnostic) !?Instruction {
...
fn encodeDataTransfer(line: *Tokenizer, diag: *Diagnostic, comptime flags: DataTransfer) !Instruction {
    ...
    if (inst.p == 0) {
        if (parsing.optionalOperator("]", line)) |_| {
            if (parsing.optionalOperator("!", line)) |_| inst.w = 1;
    ...
    _ = try parsing.expectOperator("]", line, diag);
    _ = parsing.optionalOperator("!", line) orelse return Instruction{ .encoding = @bitCast(inst) };
...
fn encodeMoveFromPsr(line: *Tokenizer, diag: *Diagnostic) !Instruction {
    return Instruction{ .encoding = @bitCast(MoveFromPsr{ .rd = try parsing.expectRegister(line, diag) }) };
}
...
fn encodeDataProcessing(mnem: u32, line: *Tokenizer, diag: *Diagnostic, flags: DataProcessing) !Instruction {
    ...
    if (mnem >= perf.LSL and mnem <= perf.LSLX) {
        ...
        const negated = if (parsing.optionalOperator("-", line)) |_| true else false;
...
fn encodeMoveImmediate(line: *Tokenizer, diag: *Diagnostic) !Instruction {
    ...
    // Test for optional '-'
    if (parsing.optionalOperator("-", line)) |_| inst.m = 1;
...
fn INST(line: []const u8) !Instruction {
    ...
    if (encodeInstruction(&tokens, &diag)) |res| {
